...
def main():
    # Parse arguments
    parser = argparse.ArgumentParser(description="Fine-tune facebook/mbart-large-50 for multilingual summarization")
    parser.add_argument("--model_name", type=str, default="facebook/mbart-large-50",
                        help="Model name or path (default: facebook/mbart-large-50)")
    ...
    parser.add_argument("--wandb_project", type=str, default="mulsum-mbart-large-50",
                        help="Weights & Biases project name (default: mulsum-mbart-large-50)")
    ...
    run_name = args.wandb_run_name or f"mbart-large-50-{args.language}-r{args.lora_r}-lr{args.learning_rate}-simple-prompt"
    wandb.init(
    ...
    # Target modules for mbart architecture
    target_modules = ["k_proj", "v_proj", "q_proj"] # Which layers to apply LoRA to
