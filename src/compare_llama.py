#!/usr/bin/env python3
"""
compare_llama.py - Qualitatively compare base and fine-tuned Llama-3.2 models
for summarization tasks by displaying side-by-side examples.
Based on the original compare_phi.py script but adapted for Llama-3.2 models.
"""
...
def create_summary_prompt(text: str, language: str) -> str:
    """Create a language-specific prompt for summarization based on the Llama-3.2 fine-tuning format"""
    # Truncate input if needed (to match input function)
    if len(text) > 4096:
        text = text[:4096]
    
    # Create appropriate prompt based on language (from finetune_llama32.py)
    if language == "en":
...
def generate_summary(model, tokenizer, text, language="en", max_input_length=4096, max_new_tokens=256, debug=False):
    ...
    # Create prompt using the format from finetune_llama32.py
    ...
    # Generate summary - using the same parameters as in finetune_llama32.py
    with torch.no_grad():
...
def main():
    # Parse arguments
    parser = argparse.ArgumentParser(description="Compare base and fine-tuned Llama-3.2 models")
    ...
    parser.add_argument("--use_4bit", action="store_true",
                        help="Load models in 4-bit quantization for memory efficiency")
    parser.add_argument("--flash_attention", action="store_true", 
                        help="Use Flash Attention 2 for faster generation (if available)")
    ...
    # Determine quantization settings based on args
    if args.use_4bit:
        logger.info("Using 4-bit quantization for models")
    ...
    # Ensure padding token is set correctly for Llama
    ...
    console.print(Panel(
        f"[bold blue]Comparing Base Llama-3.2 Model vs. Fine-tuned Llama-3.2 Model[/bold blue]\n"
    ...
    if calculate_rouge and all_metrics["base_model"]["rouge-1"] and all_metrics["finetuned_model"]["rouge-1"]:
        ...
        for model_name, metrics in all_metrics.items():
            ...
            metrics_table.add_row(
                "Base Llama-3.2 Model" if model_name == "base_model" else "Fine-tuned Llama-3.2 Model",
                f"{avg_rouge1:.4f}",
                f"{avg_rouge2:.4f}",
                f"{avg_rougeL:.4f}"
            )
