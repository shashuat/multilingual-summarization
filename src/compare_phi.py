#!/usr/bin/env python3
"""
compare_phi.py - Qualitatively compare base and fine-tuned Phi models
...
def main():
    # Parse arguments
    parser = argparse.ArgumentParser(description="Compare base and fine-tuned Phi models")
    ...
    console.print(Panel(
        f"[bold blue]Comparing Base Phi Model vs. Fine-tuned Phi Model[/bold blue]\n"
    ...
    if calculate_rouge and all_metrics["base_model"]["rouge-1"] and all_metrics["finetuned_model"]["rouge-1"]:
        ...
        for model_name, metrics in all_metrics.items():
            ...
            metrics_table.add_row(
                "Base Phi Model" if model_name == "base_model" else "Fine-tuned Phi Model",
                f"{avg_rouge1:.4f}",
                f"{avg_rouge2:.4f}",
                f"{avg_rougeL:.4f}"
            )
